...
class BatchedPopulation:
    ...
    def __init__(
        ...
        self.fc1_weight: Float[Tensor, "pop_size hidden_size input_size"] = (
            torch.randn(pop_size, hidden_size, input_size, device=config.DEVICE)
            * fc1_std
        )
        ...
        self.fc2_weight: Float[Tensor, "pop_size output_size hidden_size"] = (
            torch.randn(pop_size, output_size, hidden_size, device=config.DEVICE)
            * fc2_std
        )
    ...
    def evaluate_episodes(
        ...
        with torch.no_grad():
            total_ce: Float[Tensor, " pop_size"] = torch.zeros(
                self.pop_size, device=config.DEVICE
            )
            total_samples: int = 0
...
class CMAESPopulation:
    ...
    def __init__(
        ...
        self.mean = torch.randn(self.n_params, device=config.DEVICE) * 0.01

        # Global step size (sigma)
        self.sigma = sigma_init

        # Diagonal covariance (diagonal of C)
        self.C_diag = torch.ones(self.n_params, device=config.DEVICE)

        # Evolution paths
        self.p_sigma = torch.zeros(self.n_params, device=config.DEVICE)
        self.p_c = torch.zeros(self.n_params, device=config.DEVICE)
        ...
        self.offspring = torch.zeros(
            self.pop_size, self.n_params, device=config.DEVICE
        )

    def _compute_mu_eff(self) -> float:
        """Compute variance effectiveness of weighted recombination."""
        weights_prime = torch.log(torch.tensor(self.mu + 0.5, device=config.DEVICE)) - torch.log(
            torch.arange(1, self.mu + 1, dtype=torch.float32, device=config.DEVICE)
        )
    ...
    def _compute_weights(self) -> Float[Tensor, " mu"]:
        ...
        weights = torch.log(torch.tensor(self.mu + 0.5, device=config.DEVICE)) - torch.log(
            torch.arange(1, self.mu + 1, dtype=torch.float32, device=config.DEVICE)
        )
    ...
    def sample_offspring(self) -> None:
        ...
        for i in range(self.pop_size):
            # Sample from N(0, I)
            z = torch.randn(self.n_params, device=config.DEVICE)
    ...
    def evaluate_episodes(
        ...
        """
        with torch.no_grad():
            fitness = torch.zeros(self.pop_size, device=config.DEVICE)
    ...
    def get_state_dict(self) -> dict[str, Tensor]:
        ...
        return {
            "mean": self.mean,
            "sigma": torch.tensor(self.sigma, device=config.DEVICE),
            "C_diag": self.C_diag,
